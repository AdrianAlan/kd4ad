...
def epoch(
    ...
    for image_batch in dataloader:
        ...
        teacher.eval()
        for student in students:
            output_score = student["model"](target)
            with torch.no_grad():
                student_target = score(decoded_data, target)
...
def run_experiment(dataset, digit, epochs, batch_size, save_path):
    ...
    _, _, outlier_loader, _ = generate(
        digit, batch_size, "FMNIST" if dataset == "MNIST" else "MNIST"
    )
